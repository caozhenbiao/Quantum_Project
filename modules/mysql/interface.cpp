...
static int connect(lua_State * L){
	...
	int nRet = theODBC->connect(dns,usr,pwd);
	lua_pushinteger(L,nRet);
	return 1;
}

static int close(lua_State * L){
	int nRet = theODBC->close();
	lua_pushinteger(L,nRet);
	return 1;
}
...
#ifdef WIN32
extern "C" __declspec(dllexport) int luaopen_mysqler(lua_State *L){
#else
extern "C" int luaopen_mysqler(lua_State *L){
#endif
	printf("luaopen_mysqler\n");
	if( !theODBC ){
		theODBC = new cmyodbc;
	}
	lua_newtable(L);
