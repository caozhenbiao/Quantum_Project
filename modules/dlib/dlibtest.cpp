...
int main(int argc, char** argv)
{
	...
	//如果只取一个人，就最face面积最大哪个 face.w * face.h
	...
	cout << "image :" << sum(face_descriptors[0]) << endl;
	...
	for (auto face2 : detector(img2)) {
		auto shape2 = sp(img2, face2);
		matrix<rgb_pixel> face_chip2;
		extract_image_chip(img2, get_face_chip_details(shape2, 150, 0.25), face_chip2);
		faces2.push_back(move(face_chip2));
	}

	if (faces2.size() == 0)
	{
		cout << "No faces found in image2!" << endl;
		return 1;
	}

	std::vector<matrix<float, 0, 1>> face_descriptors2 = net(faces2);
	cout << "face descriptor for one face: " << trans(face_descriptors2[0]) << endl;


	cout << "image 2:" << sum(face_descriptors2[0]) << endl;

	cout << "sum sqrt x:" << sqrt(sum(pow( (face_descriptors[0] - face_descriptors2[0]),2)))  << endl;
	return 0;
