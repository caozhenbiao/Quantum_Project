...
DWORD CertUtils::installCaCert(char* szFileName) {
...
//¶ÁÈ¡×Ö·û´®
DWORD CertUtils::installCaCert(const std::string &strCert) {
	BYTE pBinByte[8192] = { 0 };
	unsigned long bytesRead = 0;
	unsigned long binBytes = 4096;
	CryptStringToBinaryA((LPCSTR)strCert.c_str(), strlen(strCert.c_str()), CRYPT_STRING_BASE64HEADER, pBinByte, &binBytes, NULL, NULL);
	return installCaCert(pBinByte, binBytes);
}
...
DWORD CertUtils::uninstall(const char* delCertName) {
	HCERTSTORE hSystemStore;
					   ...
					   //  the function
	{
		...
		if (strcmp(pszNameString, delCertName) == 0) {
