...
{  
    try
    {
		...
		parser.add_option("svm", "svm name.",1);
		...
		std::string svmname = get_option(parser, "svm", "object_detector.svm");
		cout << svmname << endl;
        ...
        if (parser.option("t") || parser.option("cross-validate"))
        {
            ...
            if (parser.option("t"))
            {
				...
				cout << "Saving trained detector to " << svmname << endl;
				serialize(svmname) << detector;
        ...
        ifstream fin(svmname, ios::binary);
        if (!fin)
        {
            cout << "Can't find a trained object detector file " << svmname << endl;
